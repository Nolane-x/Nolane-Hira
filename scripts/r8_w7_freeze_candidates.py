...
def main() -> None:
    ...
    for receipt_path in sorted(args.candidates.rglob("receipt.json")):
        ...
        if receipt.get("schema_version") != "r8-w7-candidate-receipt-v1":
            continue
        if receipt.get("status") != "PASS":
            raise RuntimeError("W7 candidate receipt is not PASS")
        if receipt.get("candidate") not in CANDIDATES:
            raise RuntimeError("W7 candidate identity changed")
        if receipt.get("confirm_al_exposed") is not False:
            raise RuntimeError("W7 candidate exposed CONFIRM-AL")
        if receipt.get("confirm_am_exposed") is not False:
            raise RuntimeError("W7 candidate exposed CONFIRM-AM")
        for key in (
    ...
    if names != set(CANDIDATES) or len(rows) != len(CANDIDATES):
        raise RuntimeError(
            f"expected four W7 candidates, got {sorted(names)}"
        )
    ...
    if len(train_hashes) != 1 or len(dev_hashes) != 1:
        raise RuntimeError("W7 candidates did not share exact cache provenance")

    by_name = {receipt["candidate"]: receipt for receipt, _, _ in rows}
    for name in CANDIDATES[1:]:
        row = by_name[name]
        if row["train_case_count"] != 384:
            raise RuntimeError(f"W7 {name} case budget changed")
        if row["optimizer_case_steps"] != 384 * EPOCHS:
            raise RuntimeError(f"W7 {name} optimizer budget changed")
        if row["selected_epoch"] < 1 or row["selected_epoch"] > EPOCHS:
            raise RuntimeError(f"W7 {name} selected epoch invalid")

    if by_name["frozen-w6e-control"]["selected_epoch"] != 0:
        raise RuntimeError("W7 frozen control must remain epoch 0")
    if (
        by_name["conjunctive-primary"]["optimization_seed"]
        == by_name["conjunctive-replica"]["optimization_seed"]
    ):
        raise RuntimeError("W7 primary and replica seeds must differ")
    ...
    summary = {
        "schema_version": "r8-w7-freeze-v1",
        "status": "PASS",
        "selection_authority": "DEV_AK_INDEPENDENT_PER_CANDIDATE",
