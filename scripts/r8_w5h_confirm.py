...
def main() -> None:
    ...
    if selection.get("schema_version") != "r8-w5h-selection-receipt-v1":
        raise RuntimeError("unexpected W5h selection schema")
    if selection.get("status") != "PASS":
        raise RuntimeError("W5h selection is not PASS")
    if selection.get("confirm_exposed") is not False:
        raise RuntimeError("W5h selection already exposed confirm")
    ...
    control = BalancedBindingMatcher(CONTROL)
    ...
    receipt = {
        "schema_version": "r8-w5h-confirm-v1",
        "status": "PASS",
