...
from nmd.continuous_reliability_cache import compile_w14_cache, save_w14_cache
...
def main() -> None:
    ...
    values = all_w14_text_atoms()
    overlap = sorted(values & _prior_text_atoms())
    if overlap:
        raise RuntimeError(
            f"W14 exact text atoms overlap prior authorities: {overlap}"
        )

    rows = generate_all_w14()
    if len(rows) != 2304:
        raise RuntimeError("W14 requires exactly 2304 paired views")
    if len({row.base_id for row in rows}) != 256:
        raise RuntimeError("W14 requires exactly 256 base states")
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W14 A13 weight SHA mismatch")
    ...
    cache_path = save_w14_cache(cache, args.out / "diagnostic.pt")

    domain_counts = {
        domain: len({
            row.base_id for row in rows if row.domain_id == domain
        })
        for domain in ("BV", "BW", "BX", "BY")
    }
    receipt = {
        "schema_version": "r8-w14-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
