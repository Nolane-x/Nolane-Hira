...
def main() -> None:
    ...
    values = all_w6f_values()
    overlap = sorted(values & prior_values())
    if overlap:
        raise RuntimeError(
            f"W6f diagnostic values overlap prior authorities: {overlap}"
        )
    ...
    receipt = {
        "schema_version": "r8-w6f-cache-receipt-v1",
        "status": "PASS",
        "scope": "fresh exposed diagnostic only; no training and no confirm",
        "a13_model": A13_MODEL,
        ...
        "domain_counts": cache["metadata"]["domain_counts"],
        "k_counts": cache["metadata"]["k_counts"],
