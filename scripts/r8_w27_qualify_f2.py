...
def prior_text_atoms() -> set[str]:
    ...
    return (
        ...
        | set(all_w17_text_atoms())
        | set(all_w18_text_atoms())
        | set(all_w19_text_atoms())
        | set(all_w20_text_atoms())
        | set(all_w21_text_atoms())
        | set(all_w22_text_atoms())
        | set(all_w23_text_atoms())
        | set(all_w24_text_atoms())
        | set(all_w25_text_atoms())
        | set(all_w26_text_atoms())
    )
...
def _entailment_index(config) -> int:
    ...
    for key, value in label2id.items():
        if "entail" in str(key).lower():
            return int(value)
    raise RuntimeError("W27 reference config has no identifiable entailment label")
...
def _load_reference(name: str, spec: dict[str, str]):
    ...
    if not weight.exists():
        raise RuntimeError(f"W27 {name} model.safetensors missing")
    actual_sha = file_sha256(weight)
    if actual_sha != spec["weight_sha256"]:
        raise RuntimeError(f"W27 {name} pinned weight SHA mismatch: {actual_sha}")
...
def _score_rows(rows, model, tokenizer, entailment_index: int):
    lookup = {
        row.case_id: {task: [0.0, 0.0] for task in TASK_IDS}
        for row in rows
    }
    requests = []
    for row in rows:
        for task in TASK_IDS:
            for value, hypothesis in enumerate(REFERENCE_HYPOTHESES[task]):
                requests.append((row.case_id, task, value, row.text, hypothesis))
...
def main() -> None:
    ...
    values = all_w27_text_atoms()
    overlap = sorted(values & prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W27 exact text atoms overlap prior authorities: {overlap}")
    if all_w27_query_texts() & all_w27_schema_texts():
        raise RuntimeError("W27 query/schema exact sentence overlap")

    rows = generate_w27_domains()
    if len(rows) != 384 or {row.domain_id for row in rows} != set(DOMAINS):
        raise RuntimeError("W27 authority count/domain set changed")
    ...
    result = {
        "schema_version": "r8-w27-f2-compositional-qualification-v1",
        "status": "PASS",
        **qualification,
        "panel": panel,
        ...
        "task_scoring_direction": "premise=query; hypothesis=task-option; entailment-probability",
        "panel_option_score": "arithmetic-mean-ce0-ce1-entailment-probability",
        "f2_composition": "U_AND_C",
        "case_count": 384,
        "domains": list(DOMAINS),
        "domain_seeds": dict(DOMAIN_SEEDS),
        "cases_per_domain": 96,
        "cases_per_evidence_cell_per_domain": 24,
