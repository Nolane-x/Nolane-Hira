...
def main() -> None:
    ...
    train_cache = compile_salience_cache(encoder, train)
    dev_cache = compile_salience_cache(encoder, dev)

    train_path = args.out / "train.pt"
    dev_path = args.out / "dev.pt"
    save_salience_cache(train_cache, train_path)
    save_salience_cache(dev_cache, dev_path)

    receipt = {
        "schema_version": "r8-w5g-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
        ...
        "vocab_sha256": list_hash(all_w5g_vocab()),
