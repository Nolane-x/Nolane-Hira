...
def _load_model() -> NolaneHira:
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W19 A13 weight SHA mismatch")
...
def main() -> None:
    ...
    values = all_w19_text_atoms()
    overlap = sorted(values & prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W19 exact text atoms overlap prior authorities: {overlap}")

    rows = generate_all_w19()
    if len(rows) != 288:
        raise RuntimeError("W19 authority count changed")

    model = _load_model()
    cache = compile_w19_cache(model, rows)

    args.out.mkdir(parents=True, exist_ok=True)
    path = save_w19_cache(cache, args.out / "w19.pt")
    receipt = {
        "schema_version": "r8-w19-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
