...
def _entailment_index(config) -> int:
    ...
    for key, value in label2id.items():
        if "entail" in str(key).lower():
            return int(value)
    raise RuntimeError("W28 reference config has no identifiable entailment label")
...
def _load_reference(name: str, spec: dict[str, str]):
    ...
    if not weight.exists():
        raise RuntimeError(f"W28 {name} model.safetensors missing")
    actual_sha = file_sha256(weight)
    if actual_sha != spec["weight_sha256"]:
        raise RuntimeError(f"W28 {name} pinned weight SHA mismatch: {actual_sha}")
    ...
    for row in rows:
        for task in REFERENCE_TASKS:
            for value, hypothesis in enumerate(REFERENCE_HYPOTHESES[task]):
                requests.append((row.case_id, task, value, row.severity_field, hypothesis))
...
def main() -> None:
    ...
    values = all_w28_text_atoms()
    overlap = sorted(values & prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W28 exact text atoms overlap prior authorities: {overlap}")
    ...
    if len(rows) != 192 or {row.domain_id for row in rows} != set(PARTITION_DOMAINS["qualification"]):
        raise RuntimeError("W28 qualification authority changed")
    ...
    result = {
        "schema_version": "r8-w28-reference-qualification-v1",
        "status": "PASS",
