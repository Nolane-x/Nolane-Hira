...
from nmd.semantic_consistency_authority import (
    all_w13_text_atoms,
    generate_w13_diagnostics,
)
from nmd.semantic_consistency_cache import compile_w13_cache, save_w13_cache
...
def main() -> None:
    ...
    values = all_w13_text_atoms()
    overlap = sorted(values & _prior_text_atoms())
    if overlap:
        raise RuntimeError(
            f"W13 exact text atoms overlap prior authorities: {overlap}"
        )

    rows = generate_w13_diagnostics()
    if len(rows) != 2304:
        raise RuntimeError("W13 requires exactly 2304 paired views")
    if len({row.base_id for row in rows}) != 256:
        raise RuntimeError("W13 requires exactly 256 base states")
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W13 A13 weight SHA mismatch")
    ...
    cache_path = save_w13_cache(
        cache,
        args.out / "diagnostic.pt",
    )
    ...
    domain_counts = {
        ...
        for domain in ("BR", "BS", "BT", "BU")
    }
    receipt = {
        "schema_version": "r8-w13-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
