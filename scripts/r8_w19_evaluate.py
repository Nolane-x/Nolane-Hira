...
from nmd.latent_ordinal_cache import load_w19_cache
from nmd.latent_ordinal_eval import (
    classify_w19,
    evaluate_w19,
...
def _load_scorer(freeze_path: Path, checkpoints: Path):
    ...
    if row is None:
        raise RuntimeError("W19 projection-semantic-control missing")
    ...
    if file_sha256(hira_path) != W9_HIRA_SHA256:
        raise RuntimeError("W19 frozen HIRA SHA mismatch")
    if file_sha256(scorer_path) != W9_SCORER_SHA256:
        raise RuntimeError("W19 frozen scorer SHA mismatch")
...
def _reference(cache: dict):
    ...
    if not weight.exists() or file_sha256(weight) != REFERENCE_WEIGHT_SHA256:
        raise RuntimeError("W19 pinned MiniLM SHA mismatch")
...
def main() -> None:
    ...
    cache = load_w19_cache(args.cache)
    freeze, row, scorer = _load_scorer(args.w9_freeze, args.w9_checkpoints)
    metrics = evaluate_w19(scorer, cache)
    reference, reference_sha = _reference(cache)
    classification = classify_w19(metrics, reference)

    result = {
        "schema_version": "r8-w19-audit-v1",
        "status": "PASS",
        **classification,
        "metrics": metrics,
        "reference": reference,
