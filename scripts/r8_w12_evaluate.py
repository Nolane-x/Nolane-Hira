...
from nmd.anchor_residual import classify_domain, cross_domain_outcome
from nmd.anchor_residual_authority import DOMAINS
from nmd.anchor_residual_cache import load_w12_cache
from nmd.anchor_residual_eval import evaluate_anchor_residual, reference_summary
...
def _load_w9_freeze(freeze_path: Path, checkpoints: Path):
    ...
    if not required <= set(rows):
        raise RuntimeError("W12 required W9 checkpoint set missing")
    ...
    for name in required:
        ...
        if file_sha256(hira_path) != row["hira_sha256"]:
            raise RuntimeError(f"W12 {name} HIRA SHA mismatch")
        if file_sha256(scorer_path) != row["scorer_sha256"]:
            raise RuntimeError(f"W12 {name} scorer SHA mismatch")
...
def _reference_scores(cache: dict) -> tuple[dict[str, list[float]], str]:
    ...
    if not weight.exists():
        raise RuntimeError("W12 reference model.safetensors missing")
    weight_sha = file_sha256(weight)
    if weight_sha != REFERENCE_WEIGHT_SHA256:
        raise RuntimeError("W12 pinned reference weight SHA mismatch")
...
def _domain_metrics(staged: dict, reference: dict, domain: str) -> dict[str, object]:
    return {
        "reference": reference["per_domain"][domain]["definition"],
        "anchor": staged["stages"]["A"]["per_domain"][domain]["definition"],
        "coarse": staged["stages"]["C"]["per_domain"][domain]["definition"],
        "final": staged["stages"]["F"]["per_domain"][domain]["definition"],
        "strata": staged["per_domain"][domain]["strata"],
...
def main() -> None:
    ...
    cache = load_w12_cache(args.cache)
    ...
    staged = evaluate_anchor_residual(hira, scorer, cache)
    ...
    for domain in DOMAINS:
        ...
        per_domain[domain] = {
            **classify_domain(metrics),
            "metrics": metrics,
            "spearman_margin_anchor_correct": staged["per_domain"][domain][
                "spearman_margin_anchor_correct"
            ],
            "spearman_margin_final_minus_anchor_correct": staged["per_domain"][domain][
                "spearman_margin_final_minus_anchor_correct"
            ],
            "anchor_to_coarse": staged["per_domain"][domain]["anchor_to_coarse"],
            "anchor_to_final": staged["per_domain"][domain]["anchor_to_final"],
        }

    outcome = cross_domain_outcome(per_domain)

    result = {
        "schema_version": "r8-w12-anchor-residual-v1",
        "status": "PASS",
