...
from nmd.latent_composition_authority import all_w18_text_atoms, generate_all_w18
from nmd.latent_composition_cache import compile_w18_cache, save_w18_cache
...
def _load_model() -> NolaneHira:
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W18 A13 weight SHA mismatch")
...
def main() -> None:
    ...
    values = all_w18_text_atoms()
    overlap = sorted(values & prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W18 exact text atoms overlap prior authorities: {overlap}")

    rows = generate_all_w18()
    if len(rows) != 384:
        raise RuntimeError("W18 authority count changed")

    model = _load_model()
    cache = compile_w18_cache(model, rows)

    args.out.mkdir(parents=True, exist_ok=True)
    path = save_w18_cache(cache, args.out / "w18.pt")
    receipt = {
        "schema_version": "r8-w18-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
