...
def main() -> None:
    ...
    values = all_w6i_values()
    overlap = sorted(values & prior_values())
    if overlap:
        raise RuntimeError(
            f"W6i values overlap prior authority/diagnostic values: {overlap}"
        )

    views = generate_w6i_diagnostics()
    cache = compile_w6i_cache(model, views)
    cache_path = save_w6i_cache(
        cache,
    ...
    receipt = {
        "schema_version": "r8-w6i-cache-receipt-v1",
        "status": "PASS",
        "scope": "fresh representation bridge diagnostic; no training",
        "a13_model": A13_MODEL,
        ...
        "representation_texts_per_base": cache[
            "metadata"
