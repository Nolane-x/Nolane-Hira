...
def main() -> None:
    ...
    train_cache = load_salience_cache(args.train_cache)
    dev_cache = load_salience_cache(args.dev_cache)

    random.seed(GLOBAL_SEED)
    np.random.seed(GLOBAL_SEED)
    torch.manual_seed(GLOBAL_SEED)

    matcher = ContrastiveSalienceMatcher(args.candidate)
    ...
    receipt = {
        "schema_version": "r8-w5g-candidate-receipt-v1",
        "status": "PASS",
