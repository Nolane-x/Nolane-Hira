...
from nmd.pairwise_latent_authority import DOMAIN_SEEDS, all_w20_text_atoms, generate_all_w20
from nmd.pairwise_latent_cache import compile_w20_cache, save_w20_cache
...
def _load_model() -> NolaneHira:
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W20 A13 weight SHA mismatch")
...
def main() -> None:
    ...
    values = all_w20_text_atoms()
    overlap = sorted(values & prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W20 exact text atoms overlap prior authorities: {overlap}")

    rows = generate_all_w20()
    if len(rows) != 384:
        raise RuntimeError("W20 authority count changed")

    model = _load_model()
    cache = compile_w20_cache(model, rows)

    args.out.mkdir(parents=True, exist_ok=True)
    path = save_w20_cache(cache, args.out / "w20.pt")
    receipt = {
        "schema_version": "r8-w20-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
