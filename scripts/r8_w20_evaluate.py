...
from nmd.pairwise_latent_cache import load_w20_cache
from nmd.pairwise_latent_eval import (
    classify_w20,
    evaluate_w20,
...
def _load_scorer(freeze_path: Path, checkpoints: Path):
    ...
    if row is None:
        raise RuntimeError("W20 projection-semantic-control missing")
    ...
    if file_sha256(hira_path) != W9_HIRA_SHA256:
        raise RuntimeError("W20 frozen HIRA SHA mismatch")
    if file_sha256(scorer_path) != W9_SCORER_SHA256:
        raise RuntimeError("W20 frozen scorer SHA mismatch")
...
def _reference(cache: dict):
    ...
    if not weight.exists() or file_sha256(weight) != REFERENCE_WEIGHT_SHA256:
        raise RuntimeError("W20 pinned MiniLM SHA mismatch")
    ...
    for case in cache["cases"]:
        ...
        for view_id in ("D0", "D1", "D2"):
            ...
            for pair in schemas["severity_pairs"] + schemas["confidence_pairs"]:
                texts.update(str(x) for x in pair["canonical"][view_id]["option_texts"])
                texts.update(str(x) for x in pair["swapped"][view_id]["option_texts"])
    ...
    for case in cache["cases"]:
        ...
        lookup[str(case["case_id"])] = {
            ...
            "severity_pairs": [
                {
                    "canonical": multiview_scores(
                        str(case["fields"]["severity"]),
                        pair["canonical"],
                    ),
                    "swapped": multiview_scores(
                        str(case["fields"]["severity"]),
                        pair["swapped"],
                    ),
                }
...
def main() -> None:
    ...
    cache = load_w20_cache(args.cache)
    freeze, row, scorer = _load_scorer(args.w9_freeze, args.w9_checkpoints)
    metrics = evaluate_w20(scorer, cache)
    reference, reference_sha = _reference(cache)
    classification = classify_w20(metrics, reference)

    result = {
        "schema_version": "r8-w20-audit-v1",
        "status": "PASS",
        **classification,
        "metrics": metrics,
        "reference": reference,
