...
from nmd.interface_decomposition_authority import (
    all_w11_text_atoms,
    generate_w11_diagnostics,
)
from nmd.interface_decomposition_cache import compile_w11_cache, save_w11_cache
...
def main() -> None:
    ...
    values = all_w11_text_atoms()
    overlap = sorted(values & _prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W11 exact text atoms overlap prior authorities: {overlap}")

    rows = generate_w11_diagnostics()
    if len(rows) != 1536:
        raise RuntimeError("W11 requires exactly 1536 paired views")
    if len({row.base_id for row in rows}) != 256:
        raise RuntimeError("W11 requires exactly 256 base states")
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W11 A13 weight SHA mismatch")
    ...
    cache_path = save_w11_cache(cache, args.out / "diagnostic.pt")
    ...
    receipt = {
        "schema_version": "r8-w11-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
