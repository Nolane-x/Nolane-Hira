...
def _domain_metrics(hira, scorer, cases):
    ...
    for domain in ("AG", "AH", "AI", "AJ"):
        ...
        if len(rows) != 96:
            raise RuntimeError(
                f"W7 TRAIN domain {domain} expected 96 cases, got {len(rows)}"
            )
...
def main() -> None:
    ...
    if file_sha256(args.w6e_hira) != W6E_HIRA_SHA256:
        raise RuntimeError("W7 base W6e HIRA SHA mismatch")
    if file_sha256(args.w6e_scorer) != W6E_SCORER_SHA256:
        raise RuntimeError("W7 base W6e scorer SHA mismatch")
    ...
    if len(train_cache["cases"]) != 384:
        raise RuntimeError("W7 TRAIN must contain 384 cases")
    if len(dev_cache["cases"]) != 192:
        raise RuntimeError("W7 DEV-AK must contain 192 cases")
    ...
    if count_parameters(hira) != HIRA_PARAMETER_COUNT:
        raise RuntimeError("W7 HIRA parameter count changed")
    ...
    if sum(p.numel() for p in base_scorer.parameters()) != SCORER_PARAMETER_COUNT:
        raise RuntimeError("W7 base scorer parameter count changed")

    if args.candidate in {
        "conjunctive-primary",
        "conjunctive-replica",
    }:
        scorer = ConjunctiveEvidenceScorer(base=base_scorer)
    else:
        scorer = base_scorer
    ...
    if args.candidate == "frozen-w6e-control":
        ...
        history, scorer_state, selected_metrics = train_w7_candidate(
            args.candidate,
            hira,
            scorer,
            train_cache["cases"],
    ...
    if total_scorer_count != expected_total_scorer:
        raise RuntimeError("W7 total scorer parameter contract changed")

    receipt = {
        "schema_version": "r8-w7-candidate-receipt-v1",
        "status": "PASS",
        ...
        "factor_branch_enabled": isinstance(
            scorer,
