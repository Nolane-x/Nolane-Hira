...
from nmd.regime_transfer_authority import all_w16_text_atoms, generate_all_w16
from nmd.regime_transfer_cache import compile_w16_cache, save_w16_cache
...
def main() -> None:
    ...
    values = all_w16_text_atoms()
    overlap = sorted(values & _prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W16 exact text atoms overlap prior authorities: {overlap}")

    rows = generate_all_w16()
    if len(rows) != 2304:
        raise RuntimeError("W16 requires exactly 2304 paired views")
    if len({row.base_id for row in rows}) != 256:
        raise RuntimeError("W16 requires exactly 256 bases")
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W16 A13 weight SHA mismatch")
    ...
    cache_path = save_w16_cache(cache, args.out / "diagnostic.pt")

    domain_counts = {
        domain: len({row.base_id for row in rows if row.domain_id == domain})
        for domain in ("CG", "CH", "CI", "CJ")
    }
    receipt = {
        "schema_version": "r8-w16-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
        ...
        "state_encoder_batches": cache["metadata"]["state_encoder_batches"],
        "encoded_state_text_count": cache["metadata"]["encoded_state_text_count"],
        "state_encoder_batches_per_base": cache["metadata"]["state_encoder_batches_per_base"],
        "encoded_state_texts_per_base": cache["metadata"]["encoded_state_texts_per_base"],
        "r2_additional_encoder_calls": cache["metadata"]["r2_additional_encoder_calls"],
        "prefix_token_identity_rate": cache["metadata"]["prefix_token_identity_rate"],
