...
def main() -> None:
    ...
    if selection.get("schema_version") != "r8-w5b-selection-v1":
        raise RuntimeError("unexpected W5b selection schema")
    if selection.get("status") != "PASS":
        raise RuntimeError("W5b selection is not PASS")
    if selection.get("candidate_count") != 4:
        raise RuntimeError("W5b candidate count mismatch")
    if selection.get("confirm_exposed") is not False:
        raise RuntimeError("W5b confirm was already exposed")

    selected_sha = file_sha256(args.selected_head)
    if selected_sha != selection.get("selected_head_sha256"):
        raise RuntimeError("selected W5b head SHA mismatch")
    mode = selection["selected_relation_mode"]
    ...
    print("R8_W5B_CONFIRM_GENERATION_BEGIN", flush=True)
    cases = generate_token_authority("confirm")
    if len(cases) != 192:
        raise RuntimeError("W5b confirm count mismatch")
    ...
    primary = {
        ...
        "state_once": cache["state_encode_calls"] == 192,
        "budget_k32": (
            per["32"]["candidate_budget_min"] == 32
            and per["32"]["candidate_budget_max"] == 32
        ),
        "budget_k64": (
            per["64"]["candidate_budget_min"] == 64
            and per["64"]["candidate_budget_max"] == 64
        ),
        "budget_k128": (
            per["128"]["candidate_budget_min"] == 128
            and per["128"]["candidate_budget_max"] == 128
        ),
        "budget_k255": (
            per["255"]["candidate_budget_min"] == 255
            and per["255"]["candidate_budget_max"] == 255
        ),
    }
    mechanism_gate = bool(
        selection.get("mechanism_evidence_gate")
    )
    verdict = (
        "TOKEN_SEMANTIC_PASS"
        if all(primary.values()) and mechanism_gate
        else "TOKEN_SEMANTIC_FAIL"
    )

    args.out.mkdir(parents=True, exist_ok=True)
    result = {
        "schema_version": "r8-w5b-confirm-v1",
        "status": "PASS",
        "verdict": verdict,
        "selected_relation_mode": mode,
        "selected_head_sha256": selected_sha,
        "selection_authority": "DEV_ONLY",
        "confirm_generated_after_selection": True,
        "confirm_case_count": 192,
        "confirm_state_encode_calls": cache["state_encode_calls"],
        "confirm_state_encode_calls_per_case": cache[
            "state_encode_calls_per_case"
        ],
        "metrics": metrics,
        "primary_gates": primary,
        "mechanism_evidence_gate": mechanism_gate,
        "selected_vs_pooled_accuracy_gain": selection[
            "selected_vs_pooled_accuracy_gain"
        ],
