...
from nmd.atomic_severity_cache import compile_w24_cache, save_w24_cache
...
def _load_model() -> NolaneHira:
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W24 A13 weight SHA mismatch")
...
def main() -> None:
    ...
    values = all_w24_text_atoms()
    overlap = sorted(values & prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W24 exact text atoms overlap prior authorities: {overlap}")

    query_prototype_overlap = sorted(
        all_w24_query_texts() & all_w24_prototype_texts()
    )
    if query_prototype_overlap:
        raise RuntimeError(
            f"W24 query/prototype exact sentence overlap: {query_prototype_overlap}"
        )

    rows = generate_all_w24()
    if len(rows) != 384:
        raise RuntimeError("W24 authority count changed")

    model = _load_model()
    cache = compile_w24_cache(model, rows)

    args.out.mkdir(parents=True, exist_ok=True)
    path = save_w24_cache(cache, args.out / "w24.pt")
    receipt = {
        "schema_version": "r8-w24-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
