...
def main() -> None:
    ...
    if len(structural_val_indices) != 500:
        raise RuntimeError("R22 structural validation must be 500")
    ...
    if len(matched_indices) != MATCHED_VAL_N:
        raise RuntimeError("R22 matched validation must be 1500")
    ...
    retention_train_cache = encode_dataset(
        ...
        metadata={
            **common,
            "role": "r22-train-functional-teacher-kl",
    ...
    if tangent_available:
        ...
        null_update = normalize_to_reference_l2(
            tangent.null_descent,
            delta,
        )
        raw_update = normalize_to_reference_l2(
            tangent.raw_descent,
            delta,
        )
    ...
    retained_structural_energy_pass = (
        ...
        >= MIN_RETAINED_STRUCTURAL_ENERGY
    )
    null_leakage_pass = (
        tangent_available
        and null_leakage_energy_fraction
        <= MAX_NULL_LEAKAGE_ENERGY
    )
    mechanism_validity = (
        probe_signal_valid
        ...
        and retained_structural_energy_pass
    ...
    if protected_r16_update is not None:
        state = apply_relation_update(
            r15_state,
    ...
    if null_update is not None:
        for alpha in ALPHAS:
            ...
            if not non_relation_ok:
                raise RuntimeError(
                    f"R22 null tangent changed non-relation state at alpha={alpha}"
                )
            ...
            candidate_correct = int(
                round(matched_accuracy * MATCHED_VAL_N)
            )
            zero_drop_retention_pass = finite_sample_relative_accuracy_pass(
                matched_accuracy,
                baseline_matched_accuracy,
                n=MATCHED_VAL_N,
                tolerance=MATCHED_RELATIVE_TOLERANCE,
            )
            applied = float(alpha) * null_update
            predicted_benefit = float(
                -torch.dot(structural_gradient, applied)
            )
            eligible = (
                baseline_validity
            ...
            candidate_states[float(alpha)] = state
            candidates.append(
                {
                    "family": "functional_null_structural_tangent",
                    "projection_rank": DAMAGE_RANK,
                    "alpha": alpha,
                    "applied_update_l2": float(applied.norm()),
                    "reference_update_l2": float(null_update.norm()),
                    "first_order_structural_predicted_benefit": (
                        predicted_benefit
                    ),
                    "matched": matched_metrics,
                    "matched_correct": candidate_correct,
                    "ranked_structural": structural_metrics,
                    "eligible": eligible,
    ...
    if selected_eligible:
        selected_row = min(
            ...
            key=lambda row: (
                ...
                float(row["matched"]["nll"]),
                float(row["applied_update_l2"]),
                float(row["alpha"]),
            ),
        )
        selected_state = candidate_states[float(selected_row["alpha"])]
    else:
        selected_row = None
        selected_state = {
    ...
    if selected_row is None:
        ...
        selected_zero_drop = False
    else:
        selected_matched = float(selected_row["matched"]["accuracy"])
        selected_correct = int(selected_row["matched_correct"])
        ...
        selected_zero_drop = bool(
            selected_row["zero_drop_retention_pass"]
        )
    ...
    gates = {
        ...
        "retained_structural_gradient_energy_floor": (
            MIN_RETAINED_STRUCTURAL_ENERGY
        ),
        "retained_structural_gradient_energy_pass": (
            retained_structural_energy_pass
        ),
    ...
    gates["primary_pass"] = all(
        [
            ...
            gates["retained_structural_gradient_energy_pass"],
    ...
    receipt = {
        "schema_version": "r22-functional-null-structural-tangent-v1",
        "status": "PASS",
        "evidence_scope": (
            "train-only exact-R15 structural gradient projected into a "
            "rank-32 functional teacher-KL damage nullspace at a fixed "
            "R16-delta probe, evaluated on new disjoint MultiNLI development "
            "authorities; no HANS/Breaking/XNLI/MASSIVE/Banking77/Laya/Jev "
            "model selection"
        ),
        "sources": {
            "a13_revision": A13_REVISION,
        ...
        "protocol": {
            ...
            "null_leakage_energy_ceiling": MAX_NULL_LEAKAGE_ENERGY,
            "normalization": (
                "functional-null tangent L2 normalized to exact frozen "
                "R15-to-R16 relation-delta L2"
            ),
            "baseline_matched_floor": BASELINE_MATCHED_FLOOR,
            ...
            "matched_relative_authority": (
                ...
                "means candidate correct count >= exact R15 correct count"
            ),
        ...
        "functional_damage": {
            ...
            "normalized_null_update_l2": (
                None if null_update is None else float(null_update.norm())
            ),
            "normalized_raw_update_l2": (
                None if raw_update is None else float(raw_update.norm())
            ),
        },
        "baseline": {
