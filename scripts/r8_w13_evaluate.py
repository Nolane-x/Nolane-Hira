...
from nmd.semantic_consistency_cache import load_w13_cache
from nmd.semantic_consistency_eval import (
...
def _load_w9_freeze(
    ...
    if not required <= set(rows):
        raise RuntimeError("W13 required W9 checkpoint set missing")
    ...
    for name in required:
        ...
        if file_sha256(hira_path) != row["hira_sha256"]:
            raise RuntimeError(f"W13 {name} HIRA SHA mismatch")
        if file_sha256(scorer_path) != row["scorer_sha256"]:
            raise RuntimeError(f"W13 {name} scorer SHA mismatch")
        loaded[name] = (
            row,
            hira_path,
            scorer_path,
...
def _reference_scores(
    ...
    if not weight.exists():
        raise RuntimeError(
            "W13 reference model.safetensors missing"
        )
    weight_sha = file_sha256(weight)
    if weight_sha != REFERENCE_WEIGHT_SHA256:
        raise RuntimeError(
            "W13 pinned reference weight SHA mismatch"
        )
    ...
    for base in cache["bases"]:
        ...
        for view in base["views"]:
            if view["view_id"] == "D0":
                texts.update(
                    str(text)
                    for text in view["option_texts"]
                )

    embeddings = _reference_embeddings(
        model,
        tokenizer,
        sorted(texts),
    )
...
def main() -> None:
    ...
    cache = load_w13_cache(args.cache)
    freeze, paths = _load_w9_freeze(
        args.freeze,
        args.checkpoints,
    )

    frozen_row, frozen_hira_path, _ = (
        paths["frozen-w6e-control"]
    )
    w9_row, _, w9_scorer_path = (
        paths["projection-semantic-control"]
    )
    ...
    staged = evaluate_semantic_consistency(
        hira,
        scorer,
        cache,
    )
    reference_lookup, reference_weight_sha = (
        _reference_scores(cache)
    )
    ...
    for domain in DOMAINS:
        ...
        per_domain[domain] = {
            **classify_domain(metrics),
            "metrics": metrics,
            "spearman_rank_stability_ensemble_correct": (
                staged["per_domain"][domain][
                    "spearman_rank_stability_ensemble_correct"
                ]
            ),
            "spearman_vote_count_ensemble_correct": (
                staged["per_domain"][domain][
                    "spearman_vote_count_ensemble_correct"
                ]
            ),
            "spearman_rank_stability_final_minus_ensemble": (
                staged["per_domain"][domain][
                    "spearman_rank_stability_final_minus_ensemble"
                ]
            ),
    ...
    result = {
        "schema_version": "r8-w13-semantic-consistency-v1",
        "status": "PASS",
        ...
        "consistency_meta": staged["consistency_meta"],
        "reference": reference,
        "per_domain_classification": per_domain,
        "outcome": outcome["outcome"],
        "stable_classification": outcome[
        ...
        "reference_weight_sha256": (
            reference_weight_sha
        ),
        "reference_is_hira_candidate": False,
        "w9_projection_scorer_sha256": (
            w9_row["scorer_sha256"]
        ),
        "hira_sha256": frozen_row["hira_sha256"],
        "probability_mass_max_error": staged[
            "probability_mass_max_error"
        ],
        "state_encodes_per_base": staged[
            "state_encodes_per_base"
        ],
        "cached_base_count": staged[
            "cached_base_count"
        ],
        "cached_view_count": staged[
            "cached_view_count"
        ],
