...
from nmd.anchor_residual_authority import (
    all_w12_text_atoms,
    generate_w12_diagnostics,
)
from nmd.anchor_residual_cache import compile_w12_cache, save_w12_cache
...
def main() -> None:
    ...
    values = all_w12_text_atoms()
    overlap = sorted(values & _prior_text_atoms())
    if overlap:
        raise RuntimeError(f"W12 exact text atoms overlap prior authorities: {overlap}")

    rows = generate_w12_diagnostics()
    if len(rows) != 1536:
        raise RuntimeError("W12 requires exactly 1536 paired views")
    if len({row.base_id for row in rows}) != 256:
        raise RuntimeError("W12 requires exactly 256 base states")
    ...
    if file_sha256(weight) != A13_WEIGHT_SHA256:
        raise RuntimeError("W12 A13 weight SHA mismatch")
    ...
    cache_path = save_w12_cache(cache, args.out / "diagnostic.pt")
    ...
    receipt = {
        "schema_version": "r8-w12-cache-receipt-v1",
        "status": "PASS",
        "a13_model": A13_MODEL,
