...
from .prototype_latent_authority import (
...
CACHE_SCHEMA = "r8-w21-prototype-latent-cache-v1"
...
def _content(batch, index: int) -> tuple[Tensor, Tensor]:
    ...
    if batch.token_ids is None:
        raise RuntimeError("W21 requires token IDs")
    tokens = batch.token_embeddings[index][content_mask]
    ids = batch.token_ids[index][content_mask].long()
    if tokens.shape[0] < 1:
        raise RuntimeError("W21 isolated field requires content token")
    return tokens, ids
...
def _schema_payload(schema, receipt):
    ...
    if any(value is None for value in required):
        raise RuntimeError("W21 schema token artifacts incomplete")
    return {
...
def _compile_views(model: NolaneHira, *, question_text: str, options, definitions):
    ...
    if not (identities[0] == identities[1] == identities[2]):
        raise RuntimeError("W21 abstract view option identity changed")
    return views
...
def compile_w21_cache(
    model: NolaneHira,
    rows: Sequence[PrototypeLatentAuthorityCase],
) -> dict:
    model.eval()
...
def _validate_views(views: dict, width: int) -> None:
    if set(views) != set(VIEW_IDS):
        raise ValueError("W21 requires D0/D1/D2")
    ids = [tuple(views[v]["option_ids"]) for v in VIEW_IDS]
    if not (ids[0] == ids[1] == ids[2]) or len(ids[0]) != width:
        raise ValueError("W21 abstract option identity changed")
...
def _validate_prototype_schema(schema: dict, width: int) -> None:
    ...
    if set(schema) != required:
        raise ValueError("W21 prototype schema payload changed")
    if len(tuple(schema["option_ids"])) != width:
        raise ValueError("W21 prototype count changed")
    if len(tuple(schema["option_texts"])) != width:
        raise ValueError("W21 prototype text count changed")


def validate_w21_cache(cache: dict) -> None:
    if not isinstance(cache, dict):
        raise ValueError("W21 cache must be dict")
    ...
    if not isinstance(metadata, dict) or not isinstance(schemas, dict) or not isinstance(cases, list):
        raise ValueError("W21 cache requires metadata/schemas/cases")
    if metadata.get("schema_version") != CACHE_SCHEMA:
        raise ValueError("unexpected W21 cache schema")
    if int(metadata.get("case_count", -1)) != len(cases):
        raise ValueError("W21 case count mismatch")
    if metadata.get("training_performed") is not False:
        raise ValueError("W21 training forbidden")
    if metadata.get("selection_performed") is not False:
        raise ValueError("W21 selection forbidden")
    if int(metadata.get("trainable_parameter_count", -1)) != 0:
        raise ValueError("W21 must be zero-parameter")
    if int(metadata.get("logical_state_compiles_per_case", -1)) != 1:
        raise ValueError("W21 logical state accounting changed")
    if int(metadata.get("a13_query_invocations_per_case", -1)) != 1:
        raise ValueError("W21 query invocation accounting changed")
    if int(metadata.get("encoded_query_sequences_per_case", -1)) != 2:
        raise ValueError("W21 query sequence accounting changed")
    if metadata.get("isolated_fields") != ["severity", "confidence"]:
        raise ValueError("W21 isolated fields changed")
    if int(metadata.get("prototypes_per_class", -1)) != 3:
        raise ValueError("W21 requires exactly three prototypes/class")
    if metadata.get("prototype_schema_scope") != "shared-per-domain":
        raise ValueError("W21 prototype schema scope changed")

    if set(schemas) != set(DOMAINS_ORDER):
        raise ValueError("W21 schema domains changed")
    for domain_id, pack in schemas.items():
        ...
        if tuple(pack["severity_prototype_class_index"]) != tuple(
            ...
            raise ValueError("W21 severity prototype membership changed")
        if tuple(pack["confidence_prototype_class_index"]) != tuple(
            i for i in range(3) for _ in range(3)
        ):
            raise ValueError("W21 confidence prototype membership changed")

    seen = set()
    for case in cases:
        case_id = str(case.get("case_id", ""))
        if not case_id or case_id in seen:
            raise ValueError("W21 invalid/duplicate case")
        seen.add(case_id)
        if case.get("domain_id") not in set(DOMAINS_ORDER):
            raise ValueError("W21 invalid domain")
        if int(case.get("severity", -1)) not in {0, 1, 2, 3}:
            raise ValueError("W21 invalid severity")
        if int(case.get("confidence_index", -1)) not in {0, 1, 2}:
            raise ValueError("W21 invalid confidence")
        reps = case.get("representations")
        if not isinstance(reps, dict):
            raise ValueError("W21 missing representations")
        for name in ("severity", "confidence"):
            ...
            if not isinstance(tokens, Tensor) or tokens.ndim != 2 or tokens.shape[-1] != 256:
                raise ValueError(f"W21 invalid {name} tokens")
            if not isinstance(ids, Tensor) or ids.ndim != 1 or ids.shape[0] != tokens.shape[0]:
                raise ValueError(f"W21 invalid {name} token IDs")


def save_w21_cache(cache: dict, path: str | Path) -> Path:
    validate_w21_cache(cache)
    path = Path(path)
...
def load_w21_cache(path: str | Path) -> dict:
