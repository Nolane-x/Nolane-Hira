...
CANDIDATES = (
    ...
    "idf-competitive-bidir-mean-proj128",
    "idf-competitive-bidir-logmeanexp-proj128",
)

TRAIN_TEMPLATES = (
    "w5i-train-survey-lattice",
    "w5i-train-catalog-prism",
    "w5i-train-signal-folio",
    "w5i-train-coordinate-slate",
)
DEV_TEMPLATES = (
    "w5i-dev-transit-mosaic",
    "w5i-dev-archive-compass",
)
CONFIRM_TEMPLATES = (
    "w5i-confirm-inspection-grid",
    "w5i-confirm-verifier-ribbon",
    "w5i-confirm-crosscheck-tableau",
)
...
def _vocab(split: str):
    ...
    raise ValueError(f"unknown W5i split: {split}")


def _signature_text(signature: tuple[str, str, str, str], split: str) -> str:
    a, b, c, d = signature
    if split in {"train", "dev"}:
        return f"tree {a}; spice {b}; island {c}; dance {d}"
    return f"insect {a}; herb {b}; volcano {c}; tool {d}"
...
def _render(
    ...
    a, b, c, d = signature
    if template_id == "w5i-train-survey-lattice":
        return (
            f"Survey lattice states tree={a} :: spice={b} :: island={c} :: dance={d}.",
            "Choose the candidate whose four labeled values agree with the survey.",
        )
    if template_id == "w5i-train-catalog-prism":
        return (
            f"Catalog prism maps dance {d}, island {c}, tree {a}, and spice {b}.",
            "Select the candidate preserving every mapping shown by the prism.",
        )
    if template_id == "w5i-train-signal-folio":
        return (
            f"Signal folio notes spice[{b}], tree[{a}], dance[{d}], island[{c}].",
            "Which candidate matches the complete folio assignment?",
        )
    if template_id == "w5i-train-coordinate-slate":
        return (
            f"Coordinate slate gives island→{c}; dance→{d}; spice→{b}; tree→{a}.",
            "Find the option with the identical four-coordinate assignment.",
        )
    if template_id == "w5i-dev-transit-mosaic":
        return (
            f"Transit mosaic lists tree {a}, island {c}, spice {b}, dance {d}.",
            "Locate the candidate that reproduces all mosaic entries.",
        )
    if template_id == "w5i-dev-archive-compass":
        return (
            f"Archive compass marks dance {d}, spice {b}, tree {a}, island {c}.",
            "Which candidate agrees with every compass mark?",
        )
    if template_id == "w5i-confirm-inspection-grid":
        return (
            f"Inspection grid carries insect={a} / herb={b} / volcano={c} / tool={d}.",
            "Select the candidate with the exact four-label inspection assignment.",
        )
    if template_id == "w5i-confirm-verifier-ribbon":
        return (
            f"Verifier ribbon maps tool {d}, insect {a}, herb {b}, volcano {c}.",
            "Find the candidate that preserves every verifier mapping.",
        )
    if template_id == "w5i-confirm-crosscheck-tableau":
        return (
            f"Crosscheck tableau states volcano {c}; tool {d}; insect {a}; herb {b}.",
            "Which candidate reconstructs all four tableau labels?",
        )
    raise ValueError(f"unknown W5i template: {template_id}")
...
def generate_binding_case(
    *,
    ...
    return CrossCandidateCase(
        case_id=f"w5i-{split}-{k}-{case_index:04d}",
        split=split,
    ...
    *,
    ...
    if split == "train":
        ...
        if not allow_confirm:
            raise RuntimeError(
                "W5i CONFIRM authority is sealed until post-selection evaluation"
            )
        counts, seed, templates = CONFIRM_K_COUNTS, CONFIRM_SEED, CONFIRM_TEMPLATES
    else:
        raise ValueError(f"unknown W5i split: {split}")
    rng = random.Random(seed)
    rows: list[CrossCandidateCase] = []
    index = 0
...
def compile_binding_cache(
    ...
    cases: Sequence[CrossCandidateCase],
) -> dict:
    encoder.eval()
    rows = []
    encoder_calls = 0
...
def validate_binding_cache(
    ...
    *,
    ...
    if cache.get("schema_version") != "r8-w5i-balanced-binding-cache-v1":
        raise ValueError("unexpected W5i cache schema")
    cases = cache.get("cases")
    if not isinstance(cases, list) or len(cases) != cache.get("case_count"):
        raise ValueError("W5i cache case count mismatch")
    ids: set[str] = set()
    for case in cases:
        case_id = str(case["case_id"])
        if not case_id or case_id in ids:
            raise ValueError("duplicate/invalid W5i case id")
        ids.add(case_id)
        if expected_split is not None and case["split"] != expected_split:
            raise ValueError("W5i cache split mismatch")
        k = int(case["k"])
        if not 2 <= k <= 255:
            raise ValueError("invalid W5i K")
        if not 0 <= int(case["gold_index"]) < k:
            raise ValueError("W5i gold index out of range")
...
class CrossCandidateBindingMatcher(nn.Module):
    def __init__(self, mode: str):
        super().__init__()
        if mode not in CANDIDATES:
            raise ValueError(f"unknown W5i candidate: {mode}")
        self.mode = mode
    ...
    def _competitive_adjusted_similarity(
        self,
        similarity: Tensor,
        option_mask: Tensor,
        context_mask: Tensor,
    ...
    def _forward_score(
        ...
        return weighted_mean + MIN_COVERAGE_WEIGHT * min_coverage
    ...
    def _reverse_evidence(
        ...
        *,
        ...
        valid_adjusted = adjusted.masked_fill(
            ~option_mask[:, :, None],
            -1e4,
        )
        reverse_support = valid_adjusted.max(dim=1).values
        if baseline == "mean":
            candidate_baseline = reverse_support.mean(dim=0)
        elif baseline == "logmeanexp":
            candidate_baseline = (
                torch.logsumexp(reverse_support, dim=0)
                - math.log(reverse_support.shape[0])
            )
        else:
            raise ValueError(f"unknown reverse baseline: {baseline}")
        residual = reverse_support - candidate_baseline[None, :]
        weights = context_mask.to(residual.dtype)
        return (
            (residual * weights[None, :]).sum(dim=-1)
    ...
    def forward_case(self, case: dict) -> Tensor:
        ...
        adjusted = self._competitive_adjusted_similarity(
            similarity,
            option_mask,
            context_mask,
            salience,
        )
        score = self._forward_score(adjusted, option_mask, salience)

        if self.mode == "idf-competitive-forward-proj128":
            pass
        elif self.mode == "idf-competitive-bidir-mean-proj128":
            score = score + self._reverse_evidence(
                adjusted, option_mask, context_mask, baseline="mean"
            )
        elif self.mode == "idf-competitive-bidir-logmeanexp-proj128":
            score = score + self._reverse_evidence(
...
def matcher_case_loss(matcher: CrossCandidateBindingMatcher, case: dict) -> Tensor:
...
def _accumulate_metrics(cases: Sequence[dict], logits_fn) -> dict[str, object]:
    ...
    n = len(cases)
    if n == 0:
        raise ValueError("W5i evaluator requires cases")
    return {
        "case_count": n,
        "accuracy": correct / n,
        "top5_recall": top5 / n,
...
def evaluate_matcher(
    matcher: CrossCandidateBindingMatcher,
    cache: dict,
) -> dict[str, object]:
    validate_binding_cache(cache)
    matcher.eval()
...
def train_matcher(
    ...
    *,
    ...
    if best_state is None or best_metrics is None:
        raise RuntimeError("W5i candidate produced no checkpoint")
...
def competence_gates(
    ...
    return {
        ...
        "k255_accuracy_gain_vs_control": (
            float(sel["255"]["accuracy"])
            - float(base["255"]["accuracy"]) >= 0.03
        ),
    }
...
def confirm_verdict(
    ...
    mechanism = (
        ...
        and gates["k255_accuracy_gain_vs_control"]
    )
    if control_competence:
        return "CROSS_CANDIDATE_CONTROL_ALREADY_RESCUES", gates
    if selected_competence and mechanism:
        return "CROSS_CANDIDATE_BINDING_RESCUE", gates
    if selected_competence:
        return "CROSS_CANDIDATE_BINDING_PARTIAL", gates

    overall_gain = float(selected["accuracy"]) - float(control["accuracy"])
    k255_accuracy_gain = (
        float(selected["per_k"]["255"]["accuracy"])
        - float(control["per_k"]["255"]["accuracy"])
    )
    mrr_gain = float(selected["mrr"]) - float(control["mrr"])
    if overall_gain >= 0.03 or k255_accuracy_gain >= 0.03 or mrr_gain >= 0.03:
        return "CROSS_CANDIDATE_BINDING_PARTIAL", gates
    return "CROSS_CANDIDATE_BINDING_FAIL", gates
