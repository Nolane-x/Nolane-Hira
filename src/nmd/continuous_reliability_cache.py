...
from .continuous_reliability_authority import ContinuousReliabilityView
from .runtime import NolaneHira

CACHE_SCHEMA = "r8-w14-continuous-reliability-cache-v1"
VALID_DOMAINS = {"BV", "BW", "BX", "BY"}
...
def compile_w14_cache(
    model: NolaneHira,
    rows: Sequence[ContinuousReliabilityView],
) -> dict:
    model.eval()
    by_base: dict[str, list[ContinuousReliabilityView]] = defaultdict(list)
    for row in rows:
    ...
    for base_id in sorted(by_base):
        ...
        if not (
            ...
            raise ValueError("W14 base identity mismatch")
        ...
        if memory.content_token_embeddings is None:
            raise RuntimeError("W14 requires state content tokens")
        ...
        for row in sorted(
            ...
            if any(value is None for value in required):
                raise RuntimeError("W14 schema token artifacts incomplete")
            if tuple(option.option_id for option in schema.options) != row.option_ids:
                raise RuntimeError("W14 compiled option identity changed")
    ...
    if state_calls != len(bases):
        raise RuntimeError("W14 violated one-state-encode-per-base contract")
...
def validate_w14_cache(cache: dict) -> None:
    if not isinstance(cache, dict):
        raise ValueError("W14 cache must be dict")
    metadata = cache.get("metadata")
    bases = cache.get("bases")
    if not isinstance(metadata, dict) or not isinstance(bases, list):
        raise ValueError("W14 cache requires metadata/bases")
    if metadata.get("schema_version") != CACHE_SCHEMA:
        raise ValueError("unexpected W14 cache schema")
    if int(metadata.get("base_count", -1)) != len(bases):
        raise ValueError("W14 base count mismatch")
    if float(metadata.get("state_encode_calls_per_base", -1)) != 1.0:
        raise ValueError("W14 state must encode once/base")
    ...
    for base in bases:
        ...
        if not base_id or base_id in seen_bases:
            raise ValueError("invalid/duplicate W14 base")
        seen_bases.add(base_id)
        if base.get("domain_id") not in VALID_DOMAINS:
            raise ValueError("invalid W14 domain")
        ...
        if (
            ...
            raise ValueError("W14 state content tokens invalid")
        if (
            not isinstance(segments, Tensor)
            or segments.ndim != 2
            or segments.shape[-1] != 256
        ):
            raise ValueError("W14 state segments invalid")

        views = base.get("views")
        if not isinstance(views, list) or len(views) != 9:
            raise ValueError("W14 each base requires exactly 9 views")
        ...
        for view in views:
            ...
            if len(option_ids) != k or len(set(option_ids)) != k or not 0 <= gold < k:
                raise ValueError("W14 option identity invalid")
            if (
...
def load_w14_cache(path: str | Path) -> dict:
