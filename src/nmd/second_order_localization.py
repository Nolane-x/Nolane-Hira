...
def compile_w6g_cache(
    ...
    for base_id in sorted(grouped):
        ...
        for index, role in enumerate(ROLE_KEYS):
            probe_texts.extend(
                [
                    reference.gold_signature[index],
                    reference.one_field_signatures[index][index],
                    spec.roles[index],
                ]
            )
...
def _isolated_score(scorer: CompetitiveCoarseScorer, candidate: Tensor, state: Tensor) -> Tensor:
    ...
    return coverage.mean() + MIN_COVERAGE_WEIGHT * coverage.min()
