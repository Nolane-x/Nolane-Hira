...
from .semantic_consistency_cache import validate_w13_cache
...
def _index(
    ...
    for row in records:
        ...
        if key in out:
            raise ValueError("duplicate W13 stage identity")
        out[key] = row
    return out
...
def _transition(
    ...
    if any(key not in a or key not in b for key in keys):
        raise ValueError("W13 transition lost paired identity")
    n = len(keys)
    if n == 0:
        return {
            "n": 0,
    ...
    return {
        ...
        "mean_rank_delta": sum(
            int(b[key]["rank"]) - int(a[key]["rank"])
            for key in keys
        ) / n,
        "mean_margin_delta": sum(
            float(b[key]["margin"]) - float(a[key]["margin"])
            for key in keys
        ) / n,
    }
...
def _chosen_record(
    ...
    source = ensemble if use_ensemble else final
    return {
        **source,
        "stage": stage,
    }
    ...
    *,
    ...
    if not keys:
        return {
            ...
            "ensemble_mrr": 0.0,
            "final_mrr": 0.0,
            "mean_rank_stability": 0.0,
            "mean_top3_union_size": 0.0,
            "mean_vote_count": 0.0,
        }
    n = len(keys)
    return {
        ...
        "mean_rank_stability": sum(
            float(meta[key]["rank_stability"]) for key in keys
        ) / n,
        "mean_top3_union_size": sum(
            float(meta[key]["top3_union_size"]) for key in keys
        ) / n,
        "mean_vote_count": sum(
            float(meta[key]["vote_count"]) for key in keys
        ) / n,
    }
...
def evaluate_semantic_consistency(
    ...
    for base in cache["bases"]:
        ...
        for k in K_VALUES:
            ...
            if set(views) != {"D0", "D1", "D2"}:
                raise RuntimeError("W13 missing paraphrase view")
            ...
            if not (
                ...
                raise RuntimeError("W13 paraphrase option identity changed")
            ...
            stratum = top1_consistency(top1_ids)
            top3_union = {
                option_ids[index]
                for order in orders
                for index in order[: min(3, len(order))]
            }
            vote_count = max(Counter(top1_ids).values())
            key = (str(base["base_id"]), k)
            meta[key] = {
                "stratum": stratum,
                "rank_stability": mean_pairwise_rank_stability(orders),
                "top3_union_size": len(top3_union),
    ...
    per_domain: dict[str, object] = {}
    guard_strict: list[dict[str, object]] = []
    guard_margin: list[dict[str, object]] = []
    guard_majority: list[dict[str, object]] = []
    ...
    for domain in DOMAINS:
        ...
        domain_keys = [
            key for key in ensemble_index
            if key[0] in domain_base_ids
        ]
        ...
        for bucket in ("STRICT", "MAJORITY", "SPLIT", "NON_STRICT"):
            ...
            for k in K_VALUES:
                keys = [
                    ...
                    if key[1] == k
                    and (
                        meta[key]["stratum"] == bucket
                        if bucket != "NON_STRICT"
                        else meta[key]["stratum"] != "STRICT"
                    )
                ]
                strata[bucket][str(k)] = _stratum_cell(
                    keys=keys,
                    total=64,
            ...
            strata[bucket]["pooled_primary_transition"] = _transition(
                records["E"],
                records["F"],
                pooled_primary,
            )
        ...
        for k in K_VALUES:
            ...
            n_strict = sum(
                meta[key]["stratum"] == "STRICT" for key in keys
            )
            margin_selected = countmatched_margin_ids(
                [
            ...
            for key in keys:
                ...
                f = final_index[key]
                strict = meta[key]["stratum"] == "STRICT"
                majority_or_strict = meta[key]["stratum"] != "SPLIT"
                guard_strict.append(
                    _chosen_record(e, f, strict, "G_strict_ensemble")
                )
                guard_margin.append(
                    _chosen_record(
                        e,
                        f,
                        key[0] in margin_selected,
                        "G_margin_countmatched",
                    )
                )
                guard_majority.append(
                    _chosen_record(
                        e,
                        f,
                        majority_or_strict,
        ...
        correlation_keys = [
            key for key in domain_keys if key[1] in (4, 16)
        ]
        stabilities = [
            float(meta[key]["rank_stability"]) for key in correlation_keys
        ]
        votes = [
            float(meta[key]["vote_count"]) for key in correlation_keys
        ]
        ensemble_correct = [
            1.0 if bool(ensemble_index[key]["correct"]) else 0.0
            for key in correlation_keys
        ]
        final_minus_ensemble = [
        ...
        per_domain[domain] = {
            "strata": strata,
            "spearman_rank_stability_ensemble_correct": spearman(
                stabilities,
                ensemble_correct,
            ),
            "spearman_vote_count_ensemble_correct": spearman(
                votes,
                ensemble_correct,
            ),
            "spearman_rank_stability_final_minus_ensemble": spearman(
                stabilities,
                final_minus_ensemble,
            ),
    ...
    guard_summaries = {
        "G_strict_ensemble": _summary(guard_strict),
        "G_margin_countmatched": _summary(guard_margin),
        "G_majority_ensemble": _summary(guard_majority),
    }
...
def reference_summary(
    ...
    for base in cache["bases"]:
        for view in base["views"]:
            ...
            if scores is None:
                raise ValueError(
                    f"missing W13 reference scores for {view['case_id']}"
                )
            logits = torch.tensor(scores, dtype=torch.float32)
            if logits.numel() != int(view["diagnosis_k"]):
                raise ValueError("W13 reference width mismatch")
            rows.append(
                _record(
                    base=base,
                    view=view,
