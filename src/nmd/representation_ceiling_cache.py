...
from .representation_ceiling_authority import RepresentationCeilingView
from .runtime import NolaneHira

CACHE_SCHEMA = "r8-w10-representation-ceiling-cache-v1"
...
def compile_w10_cache(
    model: NolaneHira,
    rows: Sequence[RepresentationCeilingView],
) -> dict:
    model.eval()
    by_base: dict[str, list[RepresentationCeilingView]] = defaultdict(list)
    for row in rows:
    ...
    for base_id in sorted(by_base):
        ...
        if not (
            ...
            raise ValueError("W10 base identity mismatch")
        ...
        if memory.content_token_embeddings is None:
            raise RuntimeError("W10 requires state content tokens")
        ...
        for row in sorted(base_rows, key=lambda x: (x.diagnosis_k, x.view_id)):
            ...
            if any(x is None for x in required):
                raise RuntimeError("W10 schema token artifacts incomplete")
            if tuple(option.option_id for option in schema.options) != row.option_ids:
                raise RuntimeError("W10 compiled option identity changed")
            views.append(
                {
                    "case_id": row.case_id,
                    "view_id": row.view_id,
    ...
    if state_calls != len(bases):
        raise RuntimeError("W10 violated one-state-encode-per-base contract")
...
def validate_w10_cache(cache: dict) -> None:
    if not isinstance(cache, dict):
        raise ValueError("W10 cache must be dict")
    metadata = cache.get("metadata")
    bases = cache.get("bases")
    if not isinstance(metadata, dict) or not isinstance(bases, list):
        raise ValueError("W10 cache requires metadata/bases")
    if metadata.get("schema_version") != CACHE_SCHEMA:
        raise ValueError("unexpected W10 cache schema")
    if int(metadata.get("base_count", -1)) != len(bases):
        raise ValueError("W10 base count mismatch")
    if float(metadata.get("state_encode_calls_per_base", -1)) != 1.0:
        raise ValueError("W10 state must encode once/base")
    ...
    for base in bases:
        ...
        if not base_id or base_id in seen_bases:
            raise ValueError("invalid/duplicate W10 base")
        seen_bases.add(base_id)
        if base.get("domain_id") not in {"BF", "BG", "BH", "BI"}:
            raise ValueError("invalid W10 domain")
        ...
        if not isinstance(state_tokens, Tensor) or state_tokens.ndim != 2 or state_tokens.shape[-1] != 256:
            raise ValueError("W10 state content tokens invalid")
        if not isinstance(segments, Tensor) or segments.ndim != 2 or segments.shape[-1] != 256:
            raise ValueError("W10 state segments invalid")

        views = base.get("views")
        if not isinstance(views, list) or len(views) != 6:
            raise ValueError("W10 each base requires exactly 6 views")
...
def load_w10_cache(path: str | Path) -> dict:
