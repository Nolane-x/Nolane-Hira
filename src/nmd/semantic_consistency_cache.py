...
from .semantic_consistency_authority import SemanticConsistencyView

CACHE_SCHEMA = "r8-w13-semantic-consistency-cache-v1"
VALID_DOMAINS = {"BR", "BS", "BT", "BU"}
...
def compile_w13_cache(
    model: NolaneHira,
    rows: Sequence[SemanticConsistencyView],
) -> dict:
    model.eval()
    by_base: dict[str, list[SemanticConsistencyView]] = defaultdict(list)
    for row in rows:
    ...
    for base_id in sorted(by_base):
        ...
        if not (
            ...
            raise ValueError("W13 base identity mismatch")
        ...
        if memory.content_token_embeddings is None:
            raise RuntimeError("W13 requires state content tokens")
        ...
        for row in sorted(
            ...
            if any(value is None for value in required):
                raise RuntimeError("W13 schema token artifacts incomplete")
            if tuple(option.option_id for option in schema.options) != row.option_ids:
                raise RuntimeError("W13 compiled option identity changed")
    ...
    if state_calls != len(bases):
        raise RuntimeError("W13 violated one-state-encode-per-base contract")
    ...
    validate_w13_cache(cache)
    return cache


def validate_w13_cache(cache: dict) -> None:
    if not isinstance(cache, dict):
        raise ValueError("W13 cache must be dict")
    metadata = cache.get("metadata")
    bases = cache.get("bases")
    if not isinstance(metadata, dict) or not isinstance(bases, list):
        raise ValueError("W13 cache requires metadata/bases")
    if metadata.get("schema_version") != CACHE_SCHEMA:
        raise ValueError("unexpected W13 cache schema")
    if int(metadata.get("base_count", -1)) != len(bases):
        raise ValueError("W13 base count mismatch")
    if float(metadata.get("state_encode_calls_per_base", -1)) != 1.0:
        raise ValueError("W13 state must encode once/base")
    ...
    for base in bases:
        ...
        if not base_id or base_id in seen_bases:
            raise ValueError("invalid/duplicate W13 base")
        seen_bases.add(base_id)
        if base.get("domain_id") not in VALID_DOMAINS:
            raise ValueError("invalid W13 domain")
        ...
        if (
            ...
            raise ValueError("W13 state content tokens invalid")
        if (
            not isinstance(segments, Tensor)
            or segments.ndim != 2
            or segments.shape[-1] != 256
        ):
            raise ValueError("W13 state segments invalid")

        views = base.get("views")
        if not isinstance(views, list) or len(views) != 9:
            raise ValueError("W13 each base requires exactly 9 views")
        ...
        for view in views:
            ...
            if (
                len(option_ids) != k
                or len(set(option_ids)) != k
                or not 0 <= gold < k
            ):
                raise ValueError("W13 option identity invalid")
            if (
                ...
                raise ValueError("W13 option tokens invalid")
            if (
                not isinstance(omask, Tensor)
                or omask.shape != otokens.shape[:2]
                or omask.dtype != torch.bool
            ):
                raise ValueError("W13 option mask invalid")
            if (omask.sum(-1) < 1).any():
                raise ValueError("W13 option requires content token")
            if (
                not isinstance(oids, Tensor)
                or oids.shape != omask.shape
                or oids.dtype != torch.long
            ):
                raise ValueError("W13 option token ids invalid")
            if (
                not isinstance(qtokens, Tensor)
                or qtokens.ndim != 2
                or qtokens.shape[-1] != 256
            ):
                raise ValueError("W13 question tokens invalid")
            if (
                not isinstance(qmask, Tensor)
                or qmask.shape != qtokens.shape[:1]
                or qmask.dtype != torch.bool
            ):
                raise ValueError("W13 question mask invalid")

            if k not in identities:
                identities[k] = option_ids
                golds[k] = gold
            elif option_ids != identities[k] or gold != golds[k]:
                raise ValueError("W13 paraphrase identity changed")

        if any(view_sets[k] != VALID_VIEWS for k in VALID_K):
            raise ValueError("W13 requires D0/D1/D2 at every K")

        ids4, ids8, ids16 = identities[4], identities[8], identities[16]
        if not (set(ids4) < set(ids8) < set(ids16)):
            raise ValueError("W13 nested membership failed")
        if [item for item in ids8 if item in ids4] != list(ids4):
            raise ValueError("W13 K4 order changed at K8")
        if [item for item in ids16 if item in ids8] != list(ids8):
            raise ValueError("W13 K8 order changed at K16")

    if int(metadata.get("view_count", -1)) != view_count:
        raise ValueError("W13 view count mismatch")


def save_w13_cache(cache: dict, path: str | Path) -> Path:
    validate_w13_cache(cache)
    path = Path(path)
...
def load_w13_cache(path: str | Path) -> dict:
