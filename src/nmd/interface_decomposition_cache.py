...
from .interface_decomposition_authority import InterfaceDecompositionView
from .runtime import NolaneHira

CACHE_SCHEMA = "r8-w11-interface-decomposition-cache-v1"
...
def compile_w11_cache(
    model: NolaneHira,
    rows: Sequence[InterfaceDecompositionView],
) -> dict:
    model.eval()
    by_base: dict[str, list[InterfaceDecompositionView]] = defaultdict(list)
    for row in rows:
    ...
    for base_id in sorted(by_base):
        ...
        if not (
            ...
            raise ValueError("W11 base identity mismatch")
        ...
        if memory.content_token_embeddings is None:
            raise RuntimeError("W11 requires state content tokens")
        ...
        for row in sorted(base_rows, key=lambda x: (x.diagnosis_k, x.view_id)):
            ...
            if any(x is None for x in required):
                raise RuntimeError("W11 schema token artifacts incomplete")
            if tuple(option.option_id for option in schema.options) != row.option_ids:
                raise RuntimeError("W11 compiled option identity changed")
            views.append(
                {
                    "case_id": row.case_id,
                    "view_id": row.view_id,
    ...
    if state_calls != len(bases):
        raise RuntimeError("W11 violated one-state-encode-per-base contract")
...
def validate_w11_cache(cache: dict) -> None:
    if not isinstance(cache, dict):
        raise ValueError("W11 cache must be dict")
    metadata = cache.get("metadata")
    bases = cache.get("bases")
    if not isinstance(metadata, dict) or not isinstance(bases, list):
        raise ValueError("W11 cache requires metadata/bases")
    if metadata.get("schema_version") != CACHE_SCHEMA:
        raise ValueError("unexpected W11 cache schema")
    if int(metadata.get("base_count", -1)) != len(bases):
        raise ValueError("W11 base count mismatch")
    if float(metadata.get("state_encode_calls_per_base", -1)) != 1.0:
        raise ValueError("W11 state must encode once/base")
    ...
    for base in bases:
        ...
        if not base_id or base_id in seen_bases:
            raise ValueError("invalid/duplicate W11 base")
        seen_bases.add(base_id)
        if base.get("domain_id") not in {"BJ", "BK", "BL", "BM"}:
            raise ValueError("invalid W11 domain")
        ...
        if not isinstance(state_tokens, Tensor) or state_tokens.ndim != 2 or state_tokens.shape[-1] != 256:
            raise ValueError("W11 state content tokens invalid")
        if not isinstance(segments, Tensor) or segments.ndim != 2 or segments.shape[-1] != 256:
            raise ValueError("W11 state segments invalid")

        views = base.get("views")
        if not isinstance(views, list) or len(views) != 6:
            raise ValueError("W11 each base requires exactly 6 views")
...
def load_w11_cache(path: str | Path) -> dict:
