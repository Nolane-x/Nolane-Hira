...
from .semantic_alignment_authority import (
    ALL_DOMAINS,
    K_VALUES,
    VIEW_IDS,
...
CACHE_SCHEMA = "r8-w9-semantic-alignment-cache-v1"
...
def compile_w9_cache(
    model: NolaneHira,
    rows: Sequence[SemanticAlignmentView],
) -> dict:
    model.eval()
    by_base: dict[str, list[SemanticAlignmentView]] = defaultdict(list)
    for row in rows:
        by_base[row.base_id].append(row)
    if not by_base:
        raise ValueError("W9 cache requires at least one base")
    ...
    for base_id in sorted(by_base):
        ...
        if (
            ...
            raise ValueError("W9 base identity mismatch")
        ...
        if memory.content_token_embeddings is None:
            raise RuntimeError("W9 cache requires state content tokens")
        ...
        for row in sorted(
            ...
            if any(value is None for value in required):
                raise RuntimeError("W9 schema token artifacts incomplete")
            if tuple(option.option_id for option in schema.options) != row.option_ids:
                raise RuntimeError("W9 compiled option identity changed")
    ...
    if state_calls != len(bases):
        raise RuntimeError("W9 violated one-state-encode-per-base contract")

    domains = sorted({str(base["domain_id"]) for base in bases})
    cache = {
        "metadata": {
...
def validate_w9_cache(cache: dict) -> None:
    if not isinstance(cache, dict):
        raise ValueError("W9 cache must be a dict")
    metadata = cache.get("metadata")
    bases = cache.get("bases")
    if not isinstance(metadata, dict) or not isinstance(bases, list):
        raise ValueError("W9 cache requires metadata/bases")
    if metadata.get("schema_version") != CACHE_SCHEMA:
        raise ValueError("unexpected W9 cache schema")
    if int(metadata.get("base_count", -1)) != len(bases):
        raise ValueError("W9 cache base count mismatch")
    if float(metadata.get("state_encode_calls_per_base", -1)) != 1.0:
        raise ValueError("W9 state must encode once/base")

    domains = set(metadata.get("domains", ()))
    if not domains or not domains <= set(ALL_DOMAINS):
        raise ValueError("W9 cache domain set invalid")
    ...
    for base in bases:
        ...
        if not base_id or base_id in seen_bases:
            raise ValueError("invalid/duplicate W9 base")
        seen_bases.add(base_id)
        if base.get("domain_id") not in ALL_DOMAINS:
            raise ValueError("invalid W9 domain")
        ...
        if (
            ...
            raise ValueError("W9 state segments must be [S,256]")
        if (
            ...
            raise ValueError("W9 state content tokens must be [T,256]")

        views = base.get("views")
        expected_views = len(K_VALUES) * len(VIEW_IDS)
        if not isinstance(views, list) or len(views) != expected_views:
            raise ValueError(
                f"W9 each base requires exactly {expected_views} views"
            )
        ...
        for view in views:
            ...
            if not case_id or case_id in seen_cases:
                raise ValueError("invalid/duplicate W9 case")
            seen_cases.add(case_id)

            view_id = view.get("view_id")
            k = int(view.get("diagnosis_k", -1))
            if view_id not in VIEW_IDS or k not in K_VALUES:
                raise ValueError("invalid W9 view/K")
            ...
            if (
                ...
                raise ValueError("W9 option tokens invalid")
            if (
                not isinstance(oids, Tensor)
                or oids.shape != otokens.shape[:2]
                or oids.dtype != torch.long
            ):
                raise ValueError("W9 option token IDs invalid")
            if (
                ...
                raise ValueError("W9 option content mask invalid")
            if len(option_ids) != k or len(set(option_ids)) != k:
                raise ValueError("W9 option identity invalid")
            if not 0 <= gold < k:
                raise ValueError("W9 gold index invalid")

            if k not in shape_ids:
                shape_ids[k] = option_ids
                golds[k] = gold
            elif option_ids != shape_ids[k] or gold != golds[k]:
                raise ValueError(
                    "W9 label/definition views changed candidate identity"
                )

        ids4, ids8, ids16 = shape_ids[4], shape_ids[8], shape_ids[16]
        if not (set(ids4) < set(ids8) < set(ids16)):
            raise ValueError("W9 nested membership contract failed")
        if [x for x in ids8 if x in ids4] != list(ids4):
            raise ValueError("W9 K4 relative order changed at K8")
        if [x for x in ids16 if x in ids8] != list(ids8):
            raise ValueError("W9 K8 relative order changed at K16")

    if int(metadata.get("view_count", -1)) != view_count:
        raise ValueError("W9 cache view count mismatch")


def save_w9_cache(cache: dict, path: str | Path) -> Path:
    validate_w9_cache(cache)
    path = Path(path)
...
def load_w9_cache(path: str | Path) -> dict:
